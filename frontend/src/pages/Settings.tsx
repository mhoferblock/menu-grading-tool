    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-full bg-[#006AFF] px-4 py-2 text-sm font-medium text-white hover:bg-[#0056CC]"
        >
      ...
      {showForm && (
        <div className="rounded-xl border border-[#E5E5E5] bg-white p-5 shadow-sm">
          <h3 className="mb-4 text-sm font-semibold text-[#1A1A1A]">New Builder</h3>
          <div className="grid gap-4 md:grid-cols-4">
            <div>
              <label className="mb-1.5 block text-xs font-medium text-[#8A8A8A]">Name</label>
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="Full name"
                className="w-full rounded-lg border border-[#E5E5E5] bg-[#F6F6F6] px-3 py-2 text-sm text-[#1A1A1A] placeholder-[#8A8A8A] focus:border-[#006AFF] focus:outline-none"
              />
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-medium text-[#8A8A8A]">Email</label>
              <input
                type="email"
                value={email}
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-[#8A8A8A]">Team</label>
              <div className="relative">
                <select
                  value={team}
                  onChange={(e) => setTeam(e.target.value)}
                  className="w-full appearance-none rounded-lg border border-[#E5E5E5] bg-[#F6F6F6] px-3 py-2 pr-8 text-sm text-[#1A1A1A] focus:border-[#006AFF] focus:outline-none"
                >
                  {TEAMS.map((t) => (
            ...
            <div className="flex items-end gap-2">
              <button
                ...
                className="rounded-full bg-[#006AFF] px-4 py-2 text-sm font-medium text-white hover:bg-[#0056CC] disabled:opacity-50"
              >
                {saving ? 'Saving...' : 'Save'}
              </button>
              <button
                onClick={() => setShowForm(false)}
                className="rounded-lg border border-[#E5E5E5] bg-[#F6F6F6] px-4 py-2 text-sm text-[#8A8A8A] hover:text-[#1A1A1A]"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="overflow-hidden rounded-xl border border-[#E5E5E5] bg-white shadow-sm">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {builders.map((b, i) => (
              <tr key={b.id} className={cn('border-t border-[#E5E5E5]', i % 2 === 1 && 'bg-[#F6F6F6]/50')}>
                <td className="px-4 py-3 font-medium text-[#1A1A1A]">{b.name}</td>
                <td className="px-4 py-3 text-[#8A8A8A]">{b.email}</td>
                <td className="px-4 py-3">
                  <span className="rounded-full bg-[#F6F6F6] px-2.5 py-0.5 text-xs font-medium text-[#4A4A4A]">
                    {teamLabel(b.team)}
                  </span>
                </td>
                ...
                <td className="px-4 py-3 text-right">
                  <button
                    ...
                    className="rounded p-1.5 text-[#8A8A8A] hover:bg-[#E02B1D]/10 hover:text-[#E02B1D]"
...
function GradersTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="rounded-xl border border-[#E5E5E5] bg-white p-5 shadow-sm">
          <h3 className="mb-4 text-sm font-semibold text-[#1A1A1A]">New Grader</h3>
          <div className="grid gap-4 md:grid-cols-5">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-[#8A8A8A]">Role</label>
              <div className="relative">
                <select
                  value={role}
      ...
      <div className="overflow-hidden rounded-xl border border-[#E5E5E5] bg-white shadow-sm">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {graders.map((g, i) => (
              <tr key={g.id} className={cn('border-t border-[#E5E5E5]', i % 2 === 1 && 'bg-[#F6F6F6]/50')}>
                <td className="px-4 py-3 font-medium text-[#1A1A1A]">{g.name}</td>
                <td className="px-4 py-3 text-[#8A8A8A]">{g.email}</td>
                <td className="px-4 py-3">
                  <span className="rounded-full bg-[#F6F6F6] px-2.5 py-0.5 text-xs font-medium text-[#4A4A4A]">
                    {teamLabel(g.team)}
                  </span>
                ...
                <td className="px-4 py-3">
                  <span className={cn(
                    ...
                    g.role === 'lead' ? 'bg-[#006AFF]/10 text-[#006AFF]' :
                    g.role === 'admin' ? 'bg-[#8B5CF6]/10 text-[#8B5CF6]' :
                    'bg-[#F6F6F6] text-[#4A4A4A]',
                  )}>
                    {g.role}
                  </span>
                </td>
            ...
            {graders.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-sm text-[#8A8A8A]">
